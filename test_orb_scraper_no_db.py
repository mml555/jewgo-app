#!/usr/bin/env python3
"""
Test ORB Scraper (No Database)
Test the ORB scraper functionality without database connection.
"""
...
class ORBStaticScraperTest:
    """Test version of ORB Static DOM scraper without database."""
    ...
    async def scrape_category_page(self, category_url: str) -> List[Dict]:
        ...
        try:
            ...
            for business_elem in business_elements:
                business_data = await self.extract_business_data(business_elem, kosher_type, listing_type)
                if business_data:
    ...
    async def scrape_all_categories(self) -> List[Dict]:
        ...
        try:
                    # Define ORB category URLs - based on our testing
            category_urls = [
            f"{self.base_url}/category/restaurants/",  # Dairy (238 restaurants)
            f"{self.base_url}/category/fish/",  # Fish/Pareve (13 businesses) - NEW DISCOVERY!
    ...
    async def run(self):
        ...
        try:
            logger.info("Starting ORB Static Scraper Test")
            ...
            if businesses:
                ...
                with open('orb_scraped_data.json', 'w') as f:
                    json.dump(businesses, f, indent=2)
                logger.info("✅ Data saved to 'orb_scraped_data.json'")
...
async def main():
    """Main entry point."""
    scraper = ORBStaticScraperTest()
    success = await scraper.run()
    
    if success:
        logger.info("ORB Static Scraper Test completed successfully")
    else:
        logger.error("ORB Static Scraper Test failed")
