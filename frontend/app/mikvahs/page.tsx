...
export default function MikvahsPage() {
  const router = useRouter();
  const [restaurants, setRestaurants] = useState<Restaurant[]>([]);
  const [filteredRestaurants, setFilteredRestaurants] = useState<Restaurant[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchQuery, setSearchQuery] = useState('');
  const [activeTab, setActiveTab] = useState('mikvahs');
  ...
  useEffect(() => {
    const loadRestaurants = async () => {
      try {
        ...
        // Filter for mikvah locations (you can add specific filtering logic here)
        const mikvahLocations = data.restaurants.filter(restaurant => 
          restaurant.listing_type?.toLowerCase().includes('mikvah') ||
          restaurant.name.toLowerCase().includes('mikvah')
        );
        setRestaurants(mikvahLocations);
        setFilteredRestaurants(mikvahLocations);
      } catch (error) {
        console.error('Error loading mikvah locations:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleTabChange = (tab: string) => {
    ...
    switch (tab) {
      case 'mikvahs':
        // Already on mikvahs page, just update the tab
        break;
      case 'shuls':
        router.push('/shuls');
        break;
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-gray-500">Loading mikvah locations...</div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-neutral-50">
      ...
      <div className="px-4 sm:px-6 py-4 bg-white border-b border-gray-100">
        <SmartSearch
          ...
          placeholder="Search for mikvah locations..."
      ...
      <div className="px-4 sm:px-6 py-3 bg-gray-50 text-sm text-gray-600 border-b border-gray-200">
        {filteredRestaurants.length} {filteredRestaurants.length === 1 ? 'mikvah location' : 'mikvah locations'} found
      </div>
