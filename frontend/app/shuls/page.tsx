  ...
  useEffect(() => {
    const loadRestaurants = async () => {
      try {
        ...
        // Filter for synagogue locations (you can add specific filtering logic here)
        const shulLocations = data.restaurants.filter(restaurant => 
          restaurant.listing_type?.toLowerCase().includes('synagogue') ||
          restaurant.listing_type?.toLowerCase().includes('shul') ||
          restaurant.name.toLowerCase().includes('synagogue') ||
          restaurant.name.toLowerCase().includes('shul') ||
          restaurant.name.toLowerCase().includes('temple')
        );
        setRestaurants(shulLocations);
        setFilteredRestaurants(shulLocations);
      } catch (error) {
        console.error('Error loading synagogue locations:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleTabChange = (tab: string) => {
    ...
    switch (tab) {
      case 'mikvahs':
        ...
        break;
      case 'shuls':
        // Already on shuls page, just update the tab
        break;
      case 'specials':
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-gray-500">Loading synagogue locations...</div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-neutral-50">
      ...
      <div className="px-4 sm:px-6 py-4 bg-white border-b border-gray-100">
        <SmartSearch
          ...
          placeholder="Search for synagogues..."
      ...
      <div className="px-4 sm:px-6 py-3 bg-gray-50 text-sm text-gray-600 border-b border-gray-200">
        {filteredRestaurants.length} {filteredRestaurants.length === 1 ? 'synagogue' : 'synagogues'} found
      </div>
