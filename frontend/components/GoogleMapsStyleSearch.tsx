...
import { Search, MapPin, Clock, Star, Filter, X, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function GoogleMapsStyleSearch({
  ...
  const generateSuggestions = useCallback(async (searchQuery: string) => {
    ...
    if (searchQuery.length > 2) {
      try {
        ...
        const locationSuggestions: SearchSuggestion[] = predictions.slice(0, 3).map(prediction => ({
          ...
          icon: '📍',
          color: 'bg-blue-500',
          action: () => handleLocationSelect(prediction),
          metadata: prediction
        }));
  ...
  const handleLocationSelect = useCallback(async (prediction: any) => {
    try {
      const details = await googlePlacesAPI.getPlaceDetails(prediction.place_id);
      if (details?.geometry?.location) {
        ...
        setShowSuggestions(false);
        saveRecentSearch(prediction.description);
  ...
  }, [onLocationSelect, onSearch, saveRecentSearch, handleSuggestionSelect]);
